...
public class ShardLogic : MonoBehaviour
{
    ...
    readonly Color m_Color2 = new Color(0.9f, 1.0f, 0.7f, 1.0f) * 2.0f;
    ...
    private void FixedUpdate() {
        ...
        if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out RaycastHit hit, GameManager.pickDistance, m_LayerMask)) {
            if (hit.collider.CompareTag("Shard") && hit.collider.gameObject == gameObject && m_ActionButtonIsPressed) {
                PickItem();
            }
        }
    }
    ...
    private void OnTriggerEnter(Collider other) {
        ...
        // When player walks over the shard
    ...
    // Add shard to player's inventory
    private void PickItem() {
        if (m_Picked)
            return;

        m_Picked = true;
        GameObject.FindGameObjectWithTag("ShardManager").GetComponent<ShardManager>().AddShard();
